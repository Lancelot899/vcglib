...
inline bool IsManifold( FaceType const & f, const int j )
{
  ...
  if(FaceType::HasFFAdjacency())
      return ( f.cFFp(j) == &f || &f == f.cFFp(j)->cFFp(f.cFFi(j)) ); //面j边对应的面应该是对面的面，再对面一次应该等于自己
  else
    return true;
}
...
void VFDetach(FaceType & f, int z) {
    if(f.V(z)->VFp()==&f ) { //if it is the first face detach from the begin
        int fz = f.V(z)->VFi();      //< 这一步相当于是如果这个点正好是迭代链的起点(注意遍历一个点周围的面是有一条链)，那么把这个点指向下一个面(空出当前面)
        f.V(z)->VFp() = f.VFp(fz);   //< Fp返回的是相邻面的指针，一个面正常有3个面和它相邻，表示返回的相邻的面
        f.V(z)->VFi() = f.VFi(fz);   //< Fi表示的是返回的相邻面对应的第几个点
