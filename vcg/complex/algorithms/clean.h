...
public:
  ...
  static int RemoveDuplicateVertex( MeshType & m, bool RemoveDegenerateFlag=true)    // V1.0
  {
    ...
    std::map<VertexPointer, VertexPointer> mp; //这里就是记录一下，留哪个点(同一个坐标的点只留一个)
    size_t i,j;
    VertexIterator vi;
    int deleted=0;
    int k=0;
        ...
        if (!(*fi).IsD()) {
            for (k = 0; k < (*fi).VN(); ++k) {
                if (mp.find((typename MeshType::VertexPointer)(*fi).V(k)) != mp.end()) {
                    (*fi).V(k) = &*mp[(*fi).V(k)];    // 如果发现有点被删除，就找到留下的那个点，赋值给face
                }
            }
        }
    }
        ...
        if (!(*ei).IsD()) {
            for (k = 0; k < 2; ++k) {
                if (mp.find((typename MeshType::VertexPointer)(*ei).V(k)) != mp.end()) {
                    (*ei).V(k) = &*mp[(*ei).V(k)];       // 对edge的操作同face
                }
            }
        }
    }
    // 所谓退化的面，就是面有两个点记录了同一个点
...
static int RemoveDuplicateFace(MeshType& m) {
    // SortedTriple这里会将点的id从小到大排，并且在里面定义了字典序
	//   return (v[2]!=p.v[2])?(v[2]<p.v[2]):
	// (v[1] != p.v[1]) ? (v[1] < p.v[1]) :
	//	(v[0] < p.v[0]);
  ...
  static int SplitNonManifoldVertex(MeshType& m, ScalarType moveThreshold)
  {
    ...
    ss.push();
    // 通过half-edge(pos)结构和遍历face分别统计点周围的face标记了非流形点
    CountNonManifoldVertexFF(m,true);
        ...
        if (!fi->IsD()) {
            for (int i = 0; i < 3; i++) {
                if ((*fi).V(i)->IsS() && !(*fi).V(i)->IsV()) {
                    ...
                    /// 如果点是非流形点，记录通过half-edge能遍历到的所有face,存储到
                    /// std::vector<std::pair<VertexPointer, std::vector<FaceInt> > >ToSplitVec;
                    /// 上述表中
                    do {
    ...
    /// 由于修改了点会修改别的更多的数据，比如face存储了点的指针，所以需要改face
    /// PointerUpdater这个类就是专门用于处理这些问题的
    ...
    for(fi=m.face.begin(); fi!=m.face.end();++fi)
      if (!fi->IsD())
      {
        if ((!IsManifold(*fi,0))||
            (!IsManifold(*fi,1))||
            (!IsManifold(*fi,2)))
          ToDelVec.push_back(&*fi);    //这里判断的方法是面通过边对上的面，从而一条边只能有两个面
      }
            ...
            (!IsManifold(ff,2)))
        {
          for(int j=0;j<3;++j)
            if(!face::IsBorder<FaceType>(ff,j))
              vcg::face::FFDetach<FaceType>(ff,j);  //这里用的面面表，所删除面后，需要把面面表的链表连起来
  ...
  static int RemoveFaceOutOfRangeArea(MeshType& m, ScalarType MinAreaThr=0,
     ScalarType MaxAreaThr=(std::numeric_limits<ScalarType>::max)(), bool OnlyOnSelected=false) {
    int count_fd = 0;
    MinAreaThr*=2;
    MaxAreaThr*=2;
    for(FaceIterator fi=m.face.begin(); fi!=m.face.end();++fi){
    ...
    for (fi = m.face.begin(); fi != m.face.end(); ++fi)	if (!fi->IsD()) {
        for (int i = 0; i < 3; i++) {
            if (!(*fi).V(i)->IsV()) {
                ...
                /// 这里通过pos.NextE统计了那些Face
                int starSizeFF = pos.NumberOfIncidentFaces();

                if (starSizeFF != TD[(*fi).V(i)]) {
                    if (selectVert) (*fi).V(i)->SetS();
                    nonManifoldCnt++; // 如果这个点是manifold上的点，那么他必然满足通过half-edge遍历得到的face数量等于无脑遍历face记录的face数量
                }
            }
        }
    }
